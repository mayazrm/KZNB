...
# grand mean centered for rndtzz, version with people showing variance on valenceE
rndtzz$valenceE.c <- mean(rndtzz$valenceE)
rndtzz<- within(rndtzz, {valenceE.mean = ave(valenceE, id, FUN = mean)})
rndtzz$valenceE.wc <- rndtzz$valenceE - rndtzz$valenceE.mean
rndtzz$valenceE.bc <- rndtzz$valenceE.mean - rndtzz$valenceE.c

#Basic intercept + slope model, but with betw and within versions of valenceE
mle3aa.bw <- (lmer(logrt ~ valenceE.bc + valenceE.wc + (1 + valenceE.wc| id),  data=rndtzz))
summary(mle3aa.bw)

#random intercept & slope of valence & random effects of time and word
mle5aa.bw <- (lmer(logrt ~ valenceE.bc + valenceE.wc + trial40c + (1 + valenceE + trial40c | id) + (1 | word),  data=rndtzz))
summary(mle5aa.bw)

#Note that effects in runs above are almost identical to those in the original run.

#how much variance of valence is between?
summary(lmer(valenceE ~ 1 + (1|id), data = rndtzz))
var(rndtzz$valenceE)
0.01788 + 0.21762 = 0.23569 #mixed model decomp
anova(lm(valenceE ~ 1 + idcat, data = rndtzz))

(35.189 + 273.76)/1316 - 0.21762 = 0.01714368 #total var - within var = between v (anova decomp)
0.01714368 + 0.21781 = 0.2347661 #anova model decomp
0.01714368/0.2347661 = .07302451 #7% of the variance of valenceE is between.

  # standard model -- Significant main effect of valence
mle1a <- (lm(logrt~valenceE + trial40c, data = rndtz))
summary(mle1a)
mle1b <- (lm(rtms~valenceE, data = rndtz)) #run on raw rt
summary(mle1b)
...
mle2bb <- (lmer(rtms ~ valenceE + (1 | id),  data=rndtzz))
summary(mle2bb)
confint2bb<-confint(mle2bb)

#random intercept & slope of valence; note that rndtzz is used
...
exp(6.86632)
exp(6.86632 -.5*0.16157)
exp(6.86632 + .5*0.16157)

exp(6.86632 +.5*0.16157) - exp(6.86632 -.5*0.16157)

# MLE Dot Plot for basic model with logRT
...
#Compute population prediction interval for valenceE #need to update
...
#Remove the temporal order predictor for compatibility with SPSS
mle3bb <- lmer(rtms ~ valenceE + (1 + valenceE| id),  data=rndtzz)
summary(mle3bb)
confmle3bb <- confint(mle3bb)

# MLE Dot Plot for basic model with RT in msec

ranef.mle3bb<- -162.01 + ranef(mle3bb)$id[,2] #ValencE column

quantile(ranef.mle3bb, probs=c(.025, .975))
quantile(ranef.mle3bb, probs=c(0, 1))

#Compute population prediction interval for valenceE #need to update
c(-162 - 2*143, -162 + 2*143)
#[1] -450,  120 (vs -380, 20) #Considerably wider
...
#random intercept & slope of valence & random effects of time and word
mle5aa <- (lmer(logrt ~ valenceE + trial40c + (1 + valenceE + trial40c | id) + (1 | word),  data=rndtzz))
summary(mle5aa)

cnf.fe.mle5aa <- confint(mle5aa, "theta_") #elements of lower triang RE cov mat
cnf.fe.mle5aa <- confint(mle5aa, "beta-")  #FEs

mle5bb <- (lmer(rtms ~ valenceE + trial40c + (1 + valenceE + trial40c| id) + (1 | word),  data=rndtzz))
summary(mle5bb)
