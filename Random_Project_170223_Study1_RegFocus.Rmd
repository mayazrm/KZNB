...
mle3b <- lmer(rt ~ valenceE + trial40c + (1 + valenceE| id),  data=rndtz)
summary(mle3b)
...
```{r}
summary(mle3b) # SD of valence effect = .14
datamerge <- merge(rndid, rndtz, by = "ID")

#Make small version for SPSS
keep <- c("id", "rtms", "valenceE", "prom.c", "prev.c")
...
library(ggplot2)
library(ggthemes)
valencexprom <- ggplot(prompos, aes(prom.c, rtpos)) +
  geom_ribbon(aes(ymin=rtlo, ymax=rthi), alpha = .2, fill = "darkgreen") +
  geom_line(size = 2, color = "darkgreen") +
  geom_point(data = datamerge, aes(prom.c, logrt), position = position_jitter(h=0.05, w=0.1), alpha=.5, color = valenceE)+
  labs(x = "\n Promotion Focus\n(mean centered)", y = "Log RT" )  + theme_few() + theme(text=element_text(size=20))
ggsave(valencexprom, file = "valencexprom.pdf", height = 6, width = 8)
