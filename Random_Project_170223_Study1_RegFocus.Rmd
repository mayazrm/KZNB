...
#Run as mixed-model (June 1, 2017, CI for int included in table in draft)
library(lme4)
library(lmerTest)
summary(lmer(logrt ~ valenceE + (1|idcat), data = aggs1aa))
confint(lmer(logrt ~ valenceE + (1|idcat), data = aggs1aa))
...
#Version with no random slopes (used in paper)
mle3aaa <- (lmer(logrt ~ valenceE + (1| id),  data=rndtzz))
summary(mle3aaa)
...
promneg <- data.frame(prom.c = seq(min(datamerge$prom.c, na.rm=T), max(datamerge$prom.c, na.rm=T), .05), prev.c = 0, trial40c = 0, valenceE = -.5)
