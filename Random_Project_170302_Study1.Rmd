...
rndtz$vallab <- ifelse(rndtz$valenceE==.5, "Positive", "Negative")
spaghettiplot <- ggplot(rndtz, aes(valenceE, logrt)) +
  geom_abline(res1, intercept = res1$newint, slope = res1$newslope, color="blue", lwd = 1, alpha = .5) + 
    geom_abline(res1, mapping=aes(intercept = fixef(mle5a)["(Intercept)"], slope = fixef(mle5a)[2]), color="darkblue", lwd = 3, alpha = 1) + 
  labs(x = "Valence") +
  scale_y_continuous(name="Reaction Time (log)", limits=c(6.4, 7.5)) +
