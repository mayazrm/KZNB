...
setwd("C:/Users/Niall/Dropbox/Thinkbolger Downloads");
...
makeup$test100<-makeup$test.c/100
#coefficient of variation for pref dv:
sd(makeup$pref - 1)/mean(makeup$pref - 1)
...
#Try a model with random slopes
m1a<-lmer(pref ~ est.c * att.c + prog.c * att.c + test.c * att.c + etop.c * att.c + (1 + att.c | code) + (1 | face), data=makeup, REML=FALSE)
summary(m1a)
plot(effect("att.c", m1a))
plot(effect("test.c", m1a))
plot(effect("test.c * att.c", m1a))

avPlots(m1a) #Won't run

#Try to get something to work!
m1abrm<-brm(pref ~ 1 + att.c + test100 + att.c*test100 + (1 + att.c | code), data=makeup)
summary(m1abrm)
launch_shiny(m1abrm)

m1brm<-brm(pref ~ 1 + (1 | face), data=makeup)
summary(m1brm)
