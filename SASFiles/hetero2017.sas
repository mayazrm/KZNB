...
*get rid of people with no variance;
data rtexptnew;
set hetero.rtexpt;
if subj ne 9;
if subj ne 15;
...
proc mixed data=rtexptnew covtest noclprint;
class subj time_e;
...
random  time_e*trait_vale/subject = subj type = vc g gcorr solution;
ods exclude SolutionR;
ods output SolutionR = randpred;
*lsmeans trait_vale time_e trait_vale*time_e;
run;
...
*May 20 2017;
*I'm trying to understand why SAS doesn't give standard errors for some of subject 1's estimates;
data subj1;
set hetero.rtexpt;
if subj = 1;
keep subj time_e trait_vale rt rt_log tcode; 
run;
proc sort;
by time_e trait_vale;
run;

proc freq;
tables time_e*trait_vale;
run;
proc glm;
class time_e trait_vale;
model rt_log=time_e trait_vale time_e*trait_vale/ss3;
lsmeans time_e*trait_vale;
run;
